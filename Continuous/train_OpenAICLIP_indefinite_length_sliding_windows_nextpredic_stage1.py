...
- 滑动窗口：给定 (frame_t, frame_{t+d}, frame_{t+2d}) 作为 cond，预测 frame_{t+3d}
- 必须使用 frame_mask 裁掉 padding，否则滑窗会把 0-padding 当真帧训脏
- 控制 effective batch：每个视频最多采样 K 个窗口（避免 bs_eff 爆炸）

新增：混合 dilation + curriculum
- d ∈ {1,2,3}
- 前 20% steps 固定 d=1
- 后 80% steps：d 的采样概率从 [1,0,0] 线性过渡到 [0.5,0.3,0.2]（示例）
  你也可以在 yaml 里覆盖：
    window_dilation_values: [1,2,3]
    window_dilation_probs_end: [0.5,0.3,0.2]
    window_dilation_warmup_ratio: 0.2
"""
...
def create_spatio_temporal_ids(h, w, time_step, device):
    ...
    flat_t = torch.full_like(flat_h, fill_value=int(time_step))
...
def _normalize_probs(probs):
    ...
    s = sum(probs)
    if s <= 0:
        raise ValueError(f"Invalid probs sum={s}, probs={probs}")
    return [p / s for p in probs]


def _get_list(args, key, default):
    """
    OmegaConf 里可能是 ListConfig，转成 python list
    """
    if hasattr(args, key):
        v = getattr(args, key)
        try:
...
def sample_dilation_curriculum(
    global_step: int,
    max_train_steps: int,
    d_values=(1, 2, 3),
    probs_end=(0.5, 0.3, 0.2),
    warmup_ratio: float = 0.2,
):
    """
    你要的策略：
    - 前 warmup_ratio * max_train_steps：固定 d=1
    - 后面：从 probs_start=[1,0,0] 线性过渡到 probs_end（默认 [0.5,0.3,0.2]）
    """
    ...
    probs_start = [0.0] * len(d_values)
    probs_start[0] = 1.0  # 默认把 d_values[0] 当作 d=1 的位置
    probs_start = _normalize_probs(probs_start)
    probs_end = _normalize_probs(list(probs_end))

    # 对齐长度
    if len(probs_end) != len(d_values):
        # 如果你只给了 3 个 d（默认），这里必须一致；不一致就报错更安全
        raise ValueError(f"probs_end length {len(probs_end)} != d_values length {len(d_values)}")

    if max_train_steps <= 0:
        return d_values[0]

    warmup_steps = int(max_train_steps * float(warmup_ratio))
    if global_step < warmup_steps:
        return d_values[0]

    denom = max(1, max_train_steps - warmup_steps)
    alpha = float(global_step - warmup_steps) / float(denom)
    alpha = max(0.0, min(1.0, alpha))

    probs = [(1 - alpha) * a + alpha * b for a, b in zip(probs_start, probs_end)]
    probs = _normalize_probs(probs)

    d = random.choices(d_values, weights=probs, k=1)[0]
    return int(d)
...
def build_windows_with_mask(
    ...
    """
    dilation-aware sliding windows

    frames: [B,T,C,H,W] (padded)
    frame_mask: [B,T] bool (True means valid frame)

    cond:  f[s + 0*d], f[s + 1*d], f[s + 2*d]
    target:f[s + 3*d]   (window_cond=3)
    ...
    # 需要保证 s + window_cond*d < Ti  (window_cond=3 => s+3d)
    for i in range(B):
        Ti = int(frame_mask[i].sum().item())
        if Ti < (window_cond + 1) * d:
            continue

        f = frames[i, :Ti]  # [Ti,C,H,W]

        # s 最大到 Ti - window_cond*d - 1 的前一个起点
        max_start = Ti - (window_cond * d)
        starts = list(range(0, max_start, window_stride))
        if len(starts) <= 0:
            continue
        ...
        cond0_all.append(torch.stack([f[s + 0 * d] for s in starts], dim=0))
        cond1_all.append(torch.stack([f[s + 1 * d] for s in starts], dim=0))
        cond2_all.append(torch.stack([f[s + 2 * d] for s in starts], dim=0))
        target_all.append(torch.stack([f[s + window_cond * d] for s in starts], dim=0))  # 3*d
...
def main():
    ...
    # dilation curriculum settings (can be overridden by yaml)
    dilation_values = _get_list(args, "window_dilation_values", [1, 2, 3])
    dilation_probs_end = _get_list(args, "window_dilation_probs_end", [0.5, 0.3, 0.2])
    dilation_warmup_ratio = float(getattr(args, "window_dilation_warmup_ratio", 0.2))
        ...
        for step, batch in enumerate(train_dataloader):
            try:
                ...
                with accelerator.accumulate(super_model):
                    ...
                    # ---------------------------------------------------------
                    # 0) sample dilation d (curriculum)
                    # ---------------------------------------------------------
                    d = sample_dilation_curriculum(
                        global_step=global_step,
                        max_train_steps=int(args.max_train_steps),
                        d_values=dilation_values,
                        probs_end=dilation_probs_end,
                        warmup_ratio=dilation_warmup_ratio,
                    )

                    # ---------------------------------------------------------
                    # 1) build sliding windows (mask-aware, dilation-aware)
                    # ---------------------------------------------------------
                    ...
                    ids1 = create_spatio_temporal_ids(side, side, time_step=d,     device=accelerator.device)
                    ids2 = create_spatio_temporal_ids(side, side, time_step=2 * d, device=accelerator.device)
                    ...
                    # ---------------------------------------------------------
                    # 5) img_ids: target frame time_step=3d (关键！)
                    # ---------------------------------------------------------
                    with torch.no_grad():
